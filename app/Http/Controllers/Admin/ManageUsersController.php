...
class ManageUsersController extends Controller
{
    ...
    public function detail($id)
    {
        ...
        $booked_tour = PlanLog::where('status',1)->where('type','tour')->where('user_id', $user->id)->count();
        $booked_seminar = PlanLog::where('status',1)->where('type','seminar')->where('user_id', $user->id)->count();
        $countries = json_decode(file_get_contents(resource_path('views/partials/country.json')));
        return view('admin.users.detail', compact('pageTitle', 'user','totalDeposit','countries', 'booked_seminar', 'booked_tour'));
    }
    ...
    public function deposits(Request $request, $id)
    {
        ...
        if ($request->search) {
            $search = $request->search;
            $pageTitle = 'Search User Payments : ' . $user->username;
        ...
        $pageTitle = 'User Payment : ' . $user->username;

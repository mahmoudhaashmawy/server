...
class DepositController extends Controller
{
    ...
    public function approve(Request $request)
    {
        ...
        if($deposit->plan_id){
            ...
            $plan->sold += $deposit->seat;
            $plan->save();
        }elseif ($deposit->seminar_id) {
            $seminar = Seminar::find($deposit->seminar_id);
            $seminar->sold += $deposit->seat;
            $seminar->save();
        }

        $log = PlanLog::where('status',0)->where('trx',$deposit->trx)->first();
        $log->status = 1;
        $log->save();

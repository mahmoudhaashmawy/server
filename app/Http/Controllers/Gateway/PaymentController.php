...
class PaymentController extends Controller
{
    ...
    public function deposit(Request $request)
    {
        ...
        $log = PlanLog::where('status',0)->where('id',session('log_id'))->firstOrFail();
        if (!$log) {
            $notify[] = ['error','Oops! something went wrong'];
            return back()->withNotify($notify);
        }
        ...
        return view($this->activeTemplate . 'user.payment.deposit', compact('gatewayCurrency', 'pageTitle','log'));
    }
    ...
    public function depositInsert(Request $request)
    {
        ...
        $data->plan_id = $log->type == 'tour' ? $log->plan_id : 0;
        $data->seminar_id = $log->type == 'seminar' ? $log->plan_id : 0;
        $data->seat = $log->seat;
    ...
    public static function userDataUpdate($trx)
    {
        ...
        if ($data->status == 0) {
            ...
            if($data->plan_id){
                ...
                $plan->sold += $data->seat;
                $plan->save();
            }elseif ($data->seminar_id) {
                $seminar = Seminar::find($data->seminar_id);
                $seminar->sold += $data->seat;
                $seminar->save();
            }

            $log = PlanLog::where('status',0)->where('trx',$data->trx)->first();
            $log->status = 1;
            $log->save();
    ...
    public function manualDepositUpdate(Request $request)
    {
        ...
        session()->forget(['tour_plan_id', 'seminar_id', 'seat']);
